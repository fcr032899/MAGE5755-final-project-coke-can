//
// Created by wangzhiyong on 2021/5/5.
//
...
int main(int argc, char** argv) {
    ...
    //send vision goal to find coke can:
    ROS_INFO("sending a goal: find coke can");
    g_goal_done = false;
    ...
    if (g_callback_status!= coordinator::ManipTaskResult::MANIP_SUCCESS)
    {
        ROS_ERROR("failed to find coke can quitting");
        return 0;
    }
    ...
    //send command to acquire coke can:
    ROS_INFO("sending a goal: grab coke can");
    g_goal_done = false;
    ...
    if (g_callback_status!= coordinator::ManipTaskResult::MANIP_SUCCESS)
    {
        ROS_ERROR("failed to grab coke can cs_add_executable(acquire_coke_can_client src/acquire_coke_can_client.cpp); quitting");
        return 0;
    }
