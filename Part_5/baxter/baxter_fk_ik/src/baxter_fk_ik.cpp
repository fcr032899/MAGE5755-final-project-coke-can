...
int Baxter_IK_solver::ik_solve_approx_wrt_torso(/* in */  Eigen::Affine3d const desired_tool_pose_wrt_torso,
						/* in */  Eigen::Affine3d A_tool_wrt_flange,
						/* out */ std::vector<Vectorq7x1> &q_solns) {
    //A_tool_wrt_r_arm_mount = A_flange_wrt_r_arm_mount*A_tool_wrt_flange_;
    A_tool_wrt_flange_ = A_tool_wrt_flange;
    //    A_tool_wrt_r_arm_mount = A_flange_wrt_r_arm_mount*A_tool_wrt_flange_;
...
int Baxter_IK_solver::ik_wristpt_solve_approx_wrt_torso(/* in */   Eigen::Affine3d const& desired_flange_pose_wrt_torso,
							/* out */  std::vector<Vectorq7x1> &q_solns) {
    //convert desired_hand_pose into equiv w/rt right-arm mount frame:
    //Eigen::Affine3d desired_hand_pose_wrt_arm_mount = transform_affine_from_torso_frame_to_arm_mount_frame(desired_hand_pose);
...
bool Baxter_IK_solver::improve_7dof_soln(Eigen::Affine3d const& desired_flange_pose_wrt_arm_mount,
					 Vectorq7x1 q_in,
...
bool Baxter_IK_solver::improve_7dof_soln_wrt_torso(/* in  */ Eigen::Affine3d const& desired_flange_pose_wrt_torso,
						   /* in  */ Vectorq7x1 q_in,
						   /* out */ Vectorq7x1 &q_7dof_precise) {

    // 1. Convert to frame w/rt right-arm mount
    Eigen::Affine3d desired_flange_pose_wrt_right_arm_mount = Affine_torso_to_rarm_mount_.inverse()*desired_flange_pose_wrt_torso;
    // 2. now can invoke previous fnc, w/rt right-arm mount:
